...
class Scheduler:
    ...
    def calculate_wash_time(self) -> float:
        """计算清洗时间（小时）"""
        # 简化：固定2小时
        return 2.0
    ...
    def schedule_order(self, order: CustomerOrder, state: SchedulingState) -> List[DispatchOrder]:
        ...
        """
        dispatch_orders = []
        updispatched_volume = order.undispatched_volume
        ...
        if order.id not in state.partially_scheduled_orders:
            state.partially_scheduled_orders[order.id] = updispatched_volume
    ...
    def rolling_schedule(self, orders: List[CustomerOrder], base_state: SchedulingState, 
        ...
        for cycle in range(max_cycles):
            ...
            sorted_orders = sorted(
                ...
                key=lambda o: (o.priority, o.undispatched_volume),
                reverse=True
            )
